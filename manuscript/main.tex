...
proposed $\widehat{\stab}_{\mathrm{persistence}}$ map shows a robust
ordering across tissue classes (corpus callosum~$=2.23$,
white matter~$=2.13$, grey matter~$=1.82$, CSF~$=1.70$; one-way ANOVA
$F=228.5$, $p=1.6\times 10^{-48}$; every pairwise paired
$t$-test~$p<10^{-10}$) and a positive but only moderate voxelwise
correlation with mean kurtosis (Spearman $\rho=0.39$), indicating
that the persistence-tail exponent carries non-redundant information
about microstructural restriction.
...
(\texttt{scripts/hcp\_subject\_pipeline.py}) computes, in order: (i)
...
(\texttt{sherlock/run\_hcp\_subject.sbatch}). After volume
...
$p$-values are from paired-sample $t$-tests.}
\label{tab:roi}
\small
\begin{tabular}{lccc}
\toprule
...
\multicolumn{4}{l}{\textit{Pairwise paired $t$-test $p$-values}}\\
WM vs.\ GM                         & \multicolumn{3}{l}{$2.9\times 10^{-32}$}\\
CSF vs.\ GM                        & \multicolumn{3}{l}{$1.8\times 10^{-15}$}\\
CC vs.\ WM                         & \multicolumn{3}{l}{$1.6\times 10^{-11}$}\\
CC vs.\ GM                         & \multicolumn{3}{l}{$8.6\times 10^{-26}$}\\
CSF vs.\ CC                        & \multicolumn{3}{l}{$5.2\times 10^{-25}$}\\ \bottomrule
\end{tabular}
\end{table}
